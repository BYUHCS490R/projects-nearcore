...
// NOTE: Using v1 for general use, v1beta for structured (JSON) responses
...
const vocabularyGroups = [
    ...
    { name: "Group 5: Placeholder (Empty)", items: [] },
];
...
// Global variables need to be defined outside functions for access
...
function createHeader(title, showBackButton = true) {
    ...
    if (state.currentPage === 'quizSelect') {
        ...
        if (state.currentPage === 'fillInBlanks') subtitle = "Using all vocabulary groups (1-4)";
    }
...
async function generateExamples(item) {
    ...
    const itemId = `reviewer-item-${item.hanzi}`; 

    const loadingId = `examples-loading-${item.hanzi}`;
    const detailsContainer = document.querySelector(`#${itemId} .reviewer-details-content`);
    ...
    try {
        ...
        // --- FIX: Change the highlighting from underline to bold ---
        const hanziRegex = new RegExp(item.hanzi.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1"), 'g');
        
        examples = examples.map(ex => ({
            ...ex,
            // Replace Hanzi with a bold span (using Tailwind utility class)
            hanzi: ex.hanzi.replace(hanziRegex, `<span class="font-bold text-red-600">${item.hanzi}</span>`),
            // Remove Pinyin highlighting as requested
            pinyin: ex.pinyin
        }));
        // --- END FIX ---
...
window.loadAndDisplayExamples = async (item) => {
    const itemId = `reviewer-item-${item.hanzi}`;
...
window.toggleReviewerDetails = (item) => {
    const itemElement = document.getElementById(`reviewer-item-${item.hanzi}`);
    if (!itemElement) return;
    ...
    if (!isLoaded) {
         window.loadAndDisplayExamples(item);
    } else {
...
window.checkFillInBlanksAnswer = () => {
    ...
    const updatedBlanks = currentSentence.blanks.map(blank => {
        ...
        // FIX: Ensure comparison handles different forms of input for robustness 
        const userAnswer = inputElement ? inputElement.value.trim().toLowerCase() : '';
        const correctAnswer = blank.hanzi.toLowerCase(); 

        // Simple check for Hanzi match
        const isCorrect = userAnswer === correctAnswer;
        
        if (!isCorrect) {
            allCorrect = false;
        }
        return { ...blank, userAnswer: inputElement.value.trim(), isCorrect }; // Use original case for display
    });
        ...
        const sentenceWithBlanks = sentence.hanziSentence.replace(regexPattern, (matchedWord) => {
            ...
            if (fblanks.isAnswered) {
                ...
                if (blank.isCorrect) {
                    ...
                    // FIX: Ensure user's answer is what's displayed, even if it's the wrong case/form
                    const displayValue = blank.userAnswer || '';
                    value = displayValue;
                }
            }

            // FIX: Use a more stable width calculation, or simply rely on padding/font size. 
            // 28px/char for font-size: 1.125rem (text-lg equivalent) is a reasonable estimate.
            return `
...
function renderMenu() {
    ...
    // Check if there's enough data in total for AI modes
    const allVocabPopulated = allVocabularyItems.length > 0;
        ...
        <div class="w-full max-w-md bg-white p-8 rounded-2xl shadow-2xl border border-blue-100 transition duration-300 transform hover:shadow-blue-300/50">
            ...
            ${!allVocabPopulated ? `
                <div class="mt-2 p-3 bg-red-100 border border-red-400 text-red-700 rounded-lg text-sm text-center">
                    Note: AI features require vocabulary data (Groups 1-4 are populated).
                </div>
            ` : ''}
        </div>
    `;
...
function renderReviewer() {
    ...
    const itemsHtml = group.items.map((item, index) => {
        ...
        // FIX: Use Hanzi as the reliable item ID for the DOM element
        const itemId = `reviewer-item-${item.hanzi}`;
        ...
        if (cachedExamples) {
            ...
            // FIX: Pass the item object to toggleReviewerDetails
            detailsContent = `
                <button onclick="toggleReviewerDetails(${JSON.stringify(item).replace(/'/g, "\\'")})"
        ...
        return `
            <div id="${itemId}" class="reviewer-item border-b last:border-b-0 transition duration-200">
                <div class="flex justify-between items-center p-4 hover:bg-blue-50" 
                        onclick="toggleReviewerDetails(${JSON.stringify(item).replace(/'/g, "\\'")})">
                    <div class="flex-grow">
...
function renderFlashcard() {
    ...
    // Function to handle card navigation - must be attached to window to be callable from inline HTML
    ...
    // Toggle the flip state - must be attached to window
...
/** Handles a user's answer selection during the quiz - must be attached to window */
window.handleAnswer = (selectedAnswer, buttonElement) => {
    ...
    if (!isCorrect) {
        ...
        if (correctButton) {
            // FIX: Ensure correct answer is also visually highlighted
...
/** Moves to the next question or finishes the quiz - must be attached to window */
window.nextQuestion = () => {
    if (!state.quiz.isAnswered) return;
...
/** Renders the Multiple Choice Quiz view. */
function renderQuiz() {
    ...
    const optionsHtml = currentQuestion.options.map(option => {
        // Check if this option button was the user's selected answer or the correct answer (if already answered)
        let buttonClass = 'bg-gray-200 hover:bg-gray-300';
        if (isAnswered) {
            if (option === currentQuestion.userAnswer) {
                // User's selection
                 buttonClass = currentQuestion.isCorrect ? 'correct-answer' : 'incorrect-answer';
            } else if (option === currentQuestion.correctAnswer) {
                 // Correct answer (if user was wrong)
                buttonClass = 'correct-answer opacity-70';
            } else {
                 // Unselected incorrect answer
                buttonClass = 'bg-gray-100 border-gray-300 text-gray-400 opacity-50';
            }
        }
        ...
        return `
            <button onclick="handleAnswer('${option.replace(/'/g, "\\'")}', this)"
                    class="quiz-option-btn w-full text-left p-4 my-2 border-2 border-gray-300 text-lg font-medium rounded-xl shadow-md ${buttonClass}"
                    ${isAnswered ? 'disabled' : ''}>
                ${option}
            </button>
        `;
