...
function looksMojibake(text) {
  return /Ã|Â|æ|å|ä|ç|é|ï|ð|Ç|Ä|Å|Ž|™|œ/.test(text);
}

function repairMojibake(text) {
  if (typeof text !== "string" || !looksMojibake(text)) {
    return text;
  }
  ...
  try {
    ...
    const bytes = new Uint8Array(Array.from(text, (char) => {
      const code = char.charCodeAt(0);
      if (code <= 0xFF) return code;
      if (cp1252ReverseMap[code]) return cp1252ReverseMap[code];
...
function normalizeStringsInPlace(value) {
  if (Array.isArray(value)) {
    value.forEach((item, index) => {
      if (typeof item === "string") {
        value[index] = repairMojibake(item);
      } else {
        normalizeStringsInPlace(item);
      }
    });
    return value;
  }
...
normalizeStringsInPlace(lessonSentenceBank);
normalizeStringsInPlace(hiddenSentenceDatabase);
